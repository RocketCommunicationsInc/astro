...
var gridOptions = {
  ...
  onRowGroupOpened: function (event) {
    console.log(
      "Callback onRowGroupOpened: node = " +
        event.node.key +
        ", " +
        event.expanded
    );
  ...
  processChartOptions: function (params) {
    ...
    if (type === "pie" || type === "doughnut") {
      ...
      options[isBar ? "xAxis" : "yAxis"].label.formatter =
        ...
        options.seriesDefaults.tooltip.renderer = function (params) {
          ...
          var title = params.title
            ? '<div class="ag-chart-tooltip-title"' +
              ...
              "</div>"
            : "";
          var label = params.labelKey
            ? params.datum[params.labelKey] + "<br>"
            : "";
          var xValue =
            params.xName + ": " + formatCurrency(params.datum[params.xKey]);
          var yValue =
            params.yName + ": " + formatCurrency(params.datum[params.yKey]);
          var size = "";
          if (type === "bubble" && params.sizeKey) {
            size =
              ...
              params.sizeName +
              ": " +
              formatCurrency(params.datum[params.sizeKey]);
          }
          return (
            title +
            '<div class="ag-chart-tooltip-content">' +
...
function createRowItem(row, colCount) {
  ...
  var countriesToPickFrom = Math.floor(
...
function rowSelected(event) {
  ...
  if (event.node.id < 10) {
    ...
    console.log(
      "Callback rowSelected: " + valueToPrint + " - " + event.node.isSelected()
    );
  }
}
...
PersonFloatingFilterComponent.prototype.onParentModelChanged = function (
  model
) {
...
WinningsFilter.prototype.getGui = function () {
  ...
  this.eGui.querySelectorAll("div")[1].style.backgroundColor = isDark
    ? "#222628"
    : "white";
  return this.eGui;
};
  ...
  for (var i = 0; i < 5; i++) {
    if (value > i) {
      result +=
        ...
        value +
        ' stars" class="star" width=12 height=12 />';
    }
  }
...
CountryFloatingFilterComponent.prototype.onParentModelChanged = function (
  dataModel
) {
  // add in child, one for each flat
  if (dataModel) {
...
function countryCellRenderer(params) {
  ...
  if (
    ...
    var flag =
      '<img class="flag" alt="' +
      params.value +
      '" border="0" width="15" height="10" src="https://flags.fmcdn.net/data/flags/mini/' +
