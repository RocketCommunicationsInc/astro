    ...
    return html`
      ...
      ${Object.keys(ruxIconsJson[section]).map(category => {
        return html`
          <div>
            <div class="icon-list">
            ${Object.values(ruxIconsJson[section][category]).map(icon => {
                const idx = icon.lastIndexOf('/')
                const name = icon.substring(idx + 1).replace(/\//g, '-').replace(/ /g,'-').replace(/.svg/g, '').toLowerCase()
              return html`
              <div class="icon">
                <rux-icon size="normal" icon="${name}"></rux-icon>
