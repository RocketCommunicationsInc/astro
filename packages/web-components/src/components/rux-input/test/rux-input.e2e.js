describe('Input Field with Form', () => {
    ...
    it('submits a correct file value', () => {
        cy.get('#file')
            .shadow()
            .find('input')
            .selectFile('cypress/fixtures/test.txt')
        cy.get('#formSubmitBtn').click()
        cy.get('#log').should('contain', 'test.txt')
    })
    it('maps the file attrbutes correctly', () => {
        cy.get('#file')
            .shadow()
            .find('input')
            .should('have.attr', 'accept')
            .should('eq', '.doc,image/*')
        cy.get('#file').shadow().find('input').should('have.attr', 'multiple')
        cy.get('#file')
            .shadow()
            .find('input')
