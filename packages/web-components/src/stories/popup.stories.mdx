...
<div style="display: flex; width: 100%; justify-content: center; margin-top: 10%;">
    <rux-pop-up open=${args.open} placement=${args.placement}>
...
Make use of [Menu Item](?path=/docs/components-pop-up-item--default-story) and [Menu Item Divider](?path=/docs/components-pop-up-item-divider--default-story) to compose your own Pop Up.
Every `rux-pop-up` should use the named `trigger` slot to render the trigger element for the pop-up menu.
The menu will attempt to position according to the `placement` prop, but will utlimately position where it's content 
can be seen.
