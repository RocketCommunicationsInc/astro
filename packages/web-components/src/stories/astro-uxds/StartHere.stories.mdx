import { Meta } from '@storybook/addon-docs'

<Meta title="Astro UXDS/Start Here" />

# Welcome to the Astro Storybook

This is the Astro UXDS Storybook, a sandbox for demonstrating and developing components and patterns built for
the Astro Space UX Design System ("Astro UXDS"). 

## Quick Start

For rapid prototyping, you can use our CDN to get up and running quickly. 

Add the following to your `<head>` in any html file: 

```
<link rel="preconnect" href="https://fonts.gstatic.com" />
<link
    href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700&display=swap"
    rel="stylesheet"
/>
<link
    rel="stylesheet"
    href="https://unpkg.com/@astrouxds/astro-web-components/dist/astro-web-components/astro-web-components.css"
/>
<script
    type="module"
    src="https://unpkg.com/@astrouxds/astro-web-components/dist/astro-web-components/astro-web-components.esm.js"
></script>
```

You can now use any web component on your page. 

```
<body>
    <rux-button>Hello World</rux-button>
</body>
```

## Framework Integrations

<style>
    {`
  .frameworks {
    display: grid;
    grid-template-columns: repeat(1, minmax(0, 1fr));
    gap: 2rem;
    list-style: none;
  }
  @media (min-width: 1024px) {
    .frameworks {
      grid-template-columns: repeat(3, minmax(0, 1fr));
    }
  }
  .framework {
    text-align: center;
    border-radius: var(--radius-base);
    border: 1px solid var(--color-background-interactive-default);
    cursor: pointer;
  }
  .framework .sbdocs {
    padding: 1rem;
    display: flex;
    flex-direction: column;
    align-items: center;
    font-size: 1.5rem;
  }
  .framework:hover {
    border-color: var(--color-background-interactive-hover);
  }
  .framework:hover .sbdocs-a {
    color: var(--color-background-interactive-hover);
  }
  .framework .sbdocs svg {
    margin-bottom: 2rem;
    width: 100px;
    height: 100px;
  }
`}
</style>

<ul class="frameworks">
    <li class="framework">
        <a href="?path=/docs/frameworks-javascript--page">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 630 630">
                <path fill="#f7df1e" d="M0 0h630v630H0z" />
                <path d="M423.2 492.19c12.69 20.72 29.2 35.95 58.4 35.95 24.53 0 40.2-12.26 40.2-29.2 0-20.3-16.1-27.49-43.1-39.3l-14.8-6.35c-42.72-18.2-71.1-41-71.1-89.2 0-44.4 33.83-78.2 86.7-78.2 37.64 0 64.7 13.1 84.2 47.4l-46.1 29.6c-10.15-18.2-21.1-25.37-38.1-25.37-17.34 0-28.33 11-28.33 25.37 0 17.76 11 24.95 36.4 35.95l14.8 6.34c50.3 21.57 78.7 43.56 78.7 93 0 53.3-41.87 82.5-98.1 82.5-54.98 0-90.5-26.2-107.88-60.54zm-209.13 5.13c9.3 16.5 17.76 30.45 38.1 30.45 19.45 0 31.72-7.61 31.72-37.2v-201.3h59.2v202.1c0 61.3-35.94 89.2-88.4 89.2-47.4 0-74.85-24.53-88.81-54.075z" />
            </svg>
            JavaScript
        </a>
        <a
            href="https://codesandbox.io/s/github/RocketCommunicationsInc/astro-starter-kits/tree/main/html"
            target="_blank"
        >
            <img
                alt="Edit Astro - Static HTML Starter"
                src="https://codesandbox.io/static/img/play-codesandbox.svg"
            />
        </a>
    </li>
    <li class="framework">
        <a href="?path=/docs/frameworks-angular--page">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 250 250">
                <path
                    fill="#dd0031"
                    d="M125 30 31.9 63.2l14.2 123.1L125 230l78.9-43.7 14.2-123.1z"
                />
                <path
                    fill="#c3002f"
                    d="M125 30v22.2-.1V230l78.9-43.7 14.2-123.1L125 30z"
                />
                <path
                    d="M125 52.1 66.8 182.6h21.7l11.7-29.2h49.4l11.7 29.2H183L125 52.1zm17 83.3h-34l17-40.9 17 40.9z"
                    fill="#fff"
                />
            </svg>
            Angular
        </a>
        <a
            href="https://codesandbox.io/s/github/RocketCommunicationsInc/astro-starter-kits/tree/main/angular?file=/src/app/app.component.html"
            target="_blank"
        >
            <img
                alt="Edit Astro - Angular"
                src="https://codesandbox.io/static/img/play-codesandbox.svg"
            />
        </a>
    </li>
    <li class="framework">
        <a href="?path=/docs/frameworks-react--page">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 841.9 595.3">
                <g fill="#61DAFB">
                    <path d="M666.3 296.5c0-32.5-40.7-63.3-103.1-82.4 14.4-63.6 8-114.2-20.2-130.4-6.5-3.8-14.1-5.6-22.4-5.6v22.3c4.6 0 8.3.9 11.4 2.6 13.6 7.8 19.5 37.5 14.9 75.7-1.1 9.4-2.9 19.3-5.1 29.4-19.6-4.8-41-8.5-63.5-10.9-13.5-18.5-27.5-35.3-41.6-50 32.6-30.3 63.2-46.9 84-46.9V78c-27.5 0-63.5 19.6-99.9 53.6-36.4-33.8-72.4-53.2-99.9-53.2v22.3c20.7 0 51.4 16.5 84 46.6-14 14.7-28 31.4-41.3 49.9-22.6 2.4-44 6.1-63.6 11-2.3-10-4-19.7-5.2-29-4.7-38.2 1.1-67.9 14.6-75.8 3-1.8 6.9-2.6 11.5-2.6V78.5c-8.4 0-16 1.8-22.6 5.6-28.1 16.2-34.4 66.7-19.9 130.1-62.2 19.2-102.7 49.9-102.7 82.3 0 32.5 40.7 63.3 103.1 82.4-14.4 63.6-8 114.2 20.2 130.4 6.5 3.8 14.1 5.6 22.5 5.6 27.5 0 63.5-19.6 99.9-53.6 36.4 33.8 72.4 53.2 99.9 53.2 8.4 0 16-1.8 22.6-5.6 28.1-16.2 34.4-66.7 19.9-130.1 62-19.1 102.5-49.9 102.5-82.3zm-130.2-66.7c-3.7 12.9-8.3 26.2-13.5 39.5-4.1-8-8.4-16-13.1-24-4.6-8-9.5-15.8-14.4-23.4 14.2 2.1 27.9 4.7 41 7.9zm-45.8 106.5c-7.8 13.5-15.8 26.3-24.1 38.2-14.9 1.3-30 2-45.2 2-15.1 0-30.2-.7-45-1.9-8.3-11.9-16.4-24.6-24.2-38-7.6-13.1-14.5-26.4-20.8-39.8 6.2-13.4 13.2-26.8 20.7-39.9 7.8-13.5 15.8-26.3 24.1-38.2 14.9-1.3 30-2 45.2-2 15.1 0 30.2.7 45 1.9 8.3 11.9 16.4 24.6 24.2 38 7.6 13.1 14.5 26.4 20.8 39.8-6.3 13.4-13.2 26.8-20.7 39.9zm32.3-13c5.4 13.4 10 26.8 13.8 39.8-13.1 3.2-26.9 5.9-41.2 8 4.9-7.7 9.8-15.6 14.4-23.7 4.6-8 8.9-16.1 13-24.1zM421.2 430c-9.3-9.6-18.6-20.3-27.8-32 9 .4 18.2.7 27.5.7 9.4 0 18.7-.2 27.8-.7-9 11.7-18.3 22.4-27.5 32zm-74.4-58.9c-14.2-2.1-27.9-4.7-41-7.9 3.7-12.9 8.3-26.2 13.5-39.5 4.1 8 8.4 16 13.1 24 4.7 8 9.5 15.8 14.4 23.4zM420.7 163c9.3 9.6 18.6 20.3 27.8 32-9-.4-18.2-.7-27.5-.7-9.4 0-18.7.2-27.8.7 9-11.7 18.3-22.4 27.5-32zm-74 58.9c-4.9 7.7-9.8 15.6-14.4 23.7-4.6 8-8.9 16-13 24-5.4-13.4-10-26.8-13.8-39.8 13.1-3.1 26.9-5.8 41.2-7.9zm-90.5 125.2c-35.4-15.1-58.3-34.9-58.3-50.6 0-15.7 22.9-35.6 58.3-50.6 8.6-3.7 18-7 27.7-10.1 5.7 19.6 13.2 40 22.5 60.9-9.2 20.8-16.6 41.1-22.2 60.6-9.9-3.1-19.3-6.5-28-10.2zM310 490c-13.6-7.8-19.5-37.5-14.9-75.7 1.1-9.4 2.9-19.3 5.1-29.4 19.6 4.8 41 8.5 63.5 10.9 13.5 18.5 27.5 35.3 41.6 50-32.6 30.3-63.2 46.9-84 46.9-4.5-.1-8.3-1-11.3-2.7zm237.2-76.2c4.7 38.2-1.1 67.9-14.6 75.8-3 1.8-6.9 2.6-11.5 2.6-20.7 0-51.4-16.5-84-46.6 14-14.7 28-31.4 41.3-49.9 22.6-2.4 44-6.1 63.6-11 2.3 10.1 4.1 19.8 5.2 29.1zm38.5-66.7c-8.6 3.7-18 7-27.7 10.1-5.7-19.6-13.2-40-22.5-60.9 9.2-20.8 16.6-41.1 22.2-60.6 9.9 3.1 19.3 6.5 28.1 10.2 35.4 15.1 58.3 34.9 58.3 50.6-.1 15.7-23 35.6-58.4 50.6zM320.8 78.4z" />
                    <circle cx="420.9" cy="296.5" r="45.7" />
                    <path d="M520.5 78.1z" />
                </g>
            </svg>
            React
        </a>
        <a
            target="_blank"
            href="https://codesandbox.io/s/github/RocketCommunicationsInc/astro-starter-kits/tree/main/react?fontsize=14&hidenavigation=1&theme=dark"
        >
            <img
                alt="Edit react-starter"
                src="https://codesandbox.io/static/img/play-codesandbox.svg"
            />
        </a>
    </li>
    <li class="framework">
        <a href="?path=/docs/frameworks-vue--page">
            <svg viewBox="0 0 261.76 226.69" xmlns="http://www.w3.org/2000/svg">
                <path
                    d="m161.096.001-30.224 52.35L100.647.002H-.005L130.872 226.69 261.749 0z"
                    fill="#41b883"
                />
                <path
                    d="m161.096.001-30.224 52.35L100.647.002H52.346l78.526 136.01L209.398.001z"
                    fill="#34495e"
                />
            </svg>
            Vue
        </a>
        <a
            target="_blank"
            href="https://codesandbox.io/s/github/RocketCommunicationsInc/astro-starter-kits/tree/main/vue"
        >
            <img
                alt="Edit Astro - Vue"
                src="https://codesandbox.io/static/img/play-codesandbox.svg"
            />
        </a>
    </li>
</ul>


## Extensions 

<ul class="frameworks">
    <li class="framework">
        <a href="?path=/docs/themes-ag-grid--ag-grid">
            <svg viewBox="0 0 235 40" xmlns="http://www.w3.org/2000/svg" fillRule="evenodd" clipRule="evenodd" strokeLinejoin="round" strokeMiterlimit="2">
                <path d="M205.993 25.658v-7.946h-13.541l-7.947 7.946h21.488Z" fill="#ff8b00" fillRule="nonzero"/>
                <path d="M221.25 5.95h-16.974l-8.01 7.947h24.984V5.951Z" fill="#55b2c6" fillRule="nonzero"/>
                <path d="m176.622 33.605 4.07-4.069h13.73v7.947h-17.8v-3.878Z" fill="red" fillRule="nonzero"/>
                <path d="M179.61 21.78h8.773l8.01-7.946H179.61v7.946ZM170.01 33.605h6.612l7.883-7.947h-14.494v7.947ZM204.276 5.95l-4.068 4.07H174.27V2.072h30.006V5.95Z" fill="#b4bbbf" fillRule="nonzero"/>
                <path d="M33.84 32.206H21.378l-2.162 5.34H13.75l11.507-26.954h4.768L41.53 37.546h-5.53l-2.162-5.34Zm-1.717-4.26L27.609 16.95l-4.514 10.998h9.028ZM117.437 20.509c1.589-2.352 5.594-2.607 7.247-2.607v4.578c-2.034 0-4.069.063-5.277.953-1.207.89-1.843 2.098-1.843 3.56v10.553h-4.959V17.902h4.768l.064 2.607Z" fill="#fff" fillRule="nonzero"/>
                <path fill="#fff" d="M127.418 17.902h4.958v19.644h-4.958zM127.418 9.13h4.958v5.848h-4.958z"/>
                <path d="M156.979 9.13v28.416h-4.768l-.127-2.988a8.31 8.31 0 0 1-2.67 2.48c-1.081.572-2.416.89-3.942.89-1.335 0-2.606-.255-3.687-.7-1.144-.508-2.162-1.144-2.988-2.034a9.573 9.573 0 0 1-1.97-3.179c-.51-1.207-.7-2.606-.7-4.132 0-1.525.254-2.924.7-4.196.508-1.271 1.144-2.352 1.97-3.242.826-.89 1.844-1.589 2.988-2.098 1.144-.508 2.352-.762 3.687-.762 1.526 0 2.797.254 3.878.826 1.08.572 1.97 1.335 2.67 2.416V9.193h4.959v-.064Zm-10.426 24.538c1.653 0 2.924-.572 3.941-1.653 1.017-1.08 1.526-2.542 1.526-4.322 0-1.78-.509-3.18-1.526-4.323-1.017-1.081-2.288-1.653-3.941-1.653-1.59 0-2.925.572-3.878 1.653-1.017 1.08-1.526 2.543-1.526 4.323 0 1.78.509 3.178 1.526 4.259 1.017 1.144 2.288 1.716 3.878 1.716M70.202 22.352H57.933v4.26h6.993c-.19 2.161-1.017 3.814-2.416 5.15-1.398 1.27-3.178 1.906-5.467 1.906-1.271 0-2.48-.254-3.497-.699a7.025 7.025 0 0 1-2.733-1.97c-.763-.827-1.335-1.844-1.78-3.052-.445-1.208-.636-2.48-.636-3.942 0-1.462.19-2.733.636-3.941.381-1.208 1.017-2.162 1.78-3.052.763-.826 1.653-1.462 2.733-1.97a9.115 9.115 0 0 1 3.56-.7c2.67 0 4.705.636 6.103 1.908l3.306-3.306c-2.48-1.907-5.658-2.925-9.408-2.925-2.098 0-4.006.318-5.722 1.018-1.716.699-3.179 1.589-4.386 2.797A12.074 12.074 0 0 0 44.2 18.22c-.635 1.717-.953 3.624-.953 5.658 0 2.035.318 3.942 1.017 5.658.7 1.717 1.59 3.179 2.797 4.387a12.074 12.074 0 0 0 4.387 2.797c1.716.7 3.623 1.017 5.658 1.017 2.034 0 3.877-.318 5.53-1.017 1.653-.7 3.052-1.59 4.196-2.797 1.144-1.208 2.034-2.67 2.67-4.387.636-1.716.954-3.623.954-5.658v-.826c-.191-.127-.255-.445-.255-.7M108.982 22.352h-12.27v4.26h6.993c-.19 2.161-1.017 3.814-2.416 5.15-1.398 1.27-3.178 1.906-5.467 1.906-1.271 0-2.48-.254-3.496-.699a7.025 7.025 0 0 1-2.734-1.97c-.763-.827-1.335-1.844-1.78-3.052-.445-1.208-.636-2.48-.636-3.942 0-1.462.191-2.733.636-3.941.381-1.208 1.017-2.162 1.78-3.052.763-.826 1.653-1.462 2.734-1.97a9.115 9.115 0 0 1 3.56-.7c2.67 0 4.704.636 6.103 1.908l3.305-3.306c-2.479-1.907-5.658-2.925-9.408-2.925-2.098 0-4.005.318-5.722 1.018-1.716.699-3.178 1.589-4.386 2.797a12.074 12.074 0 0 0-2.798 4.386c-.635 1.717-.953 3.624-.953 5.658 0 2.035.318 3.942 1.017 5.658.7 1.717 1.59 3.179 2.797 4.387a12.074 12.074 0 0 0 4.387 2.797c1.716.7 3.623 1.017 5.658 1.017 2.034 0 3.878-.318 5.53-1.017 1.653-.7 3.052-1.59 4.196-2.797 1.145-1.208 2.035-2.67 2.67-4.387.636-1.716.954-3.623.954-5.658v-.826c-.19-.127-.254-.445-.254-.7" fill="#fff" fillRule="nonzero"/>
            </svg> 
            AG Grid Theme
        </a>
    </li>
    <li class="framework">
        <a href="?path=/docs/themes-tailwind--page">
            <svg width="1000" height="1000" viewBox="0 0 1000 1000" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M489.5 226.499C328 231.632 280 346.999 269 409.499C283.333 386.332 328.5 335.5 395 335.5C472.5 335.5 531.5 422 567.5 449C611.237 481.803 699.123 525.115 814.5 490C906.5 462 949.167 364.332 958.5 317.999C914 378.499 846.5 414.838 763 371.999C705.5 342.499 662.5 221 489.5 226.499Z" fill="#07B6D5"/>
                <path d="M261 500.999C99.5 506.132 51.5 621.499 40.5 683.999C54.8333 660.832 100 610 166.5 610C244 610 303 696.5 339 723.5C382.737 756.303 470.623 799.615 586 764.5C678 736.5 720.667 638.832 730 592.499C685.5 652.999 618 689.338 534.5 646.499C477 616.999 434 495.5 261 500.999Z" fill="#07B6D5"/>
            </svg>
            Tailwind Config
        </a>
        
    </li>
    
    
</ul>
