...
import style from './rux-tabs.scss?inline'
...
export class RuxTabs extends LitElement {
    ...
    constructor() {
        ...
        this.addEventListener('keypress', (e) => this._onPress(e))
    ...
    private _handleCompact() {
        if (this._tabs) {
            this._tabs.forEach((tab: HTMLRuxTabElement) => tab.setAttribute('compact', ''))
        }
    }
    ...
    onKeydown(e: KeyboardEvent) {
        ...
        if (e.target && tabs.includes(e.target as HTMLRuxTabElement)) {
            const index = tabs.findIndex((tab) => tab === (e.target as HTMLRuxTabElement))
            ...
            if (next && tabs.includes(next)) {
                const nextFocus = next.shadowRoot?.querySelector('.rux-tab') as HTMLElement
                if (nextFocus) {
                    nextFocus.focus()
    ...
    private _addTabs() {
        ...
        if (slot) {
            ...
            this._tabs = slot.assignedElements({ flatten: true }).filter((el) => el.tagName.toLowerCase() === 'rux-tab') as HTMLRuxTabElement[]
        } else {
            this._tabs = []
        }
    }
    ...
    private _checkSelected() {
        ...
        const firstTabShadowContainer = firstTabInstance.shadowRoot?.querySelector('[part="container"]') as HTMLElement | null

        if (firstTabShadowContainer) {
            firstTabShadowContainer.tabIndex = !this._tabs.find((tab) => tab.selected) || this._tabs[0].hasAttribute('selected') ? 0 : -1
        }
    }
    ...
    private _reset() {
        ...
        this._panels.forEach((panel) => {
            if (panel.parentElement?.getAttribute('aria-labelledby') === this.id) {
    ...
    render() {
        return html`<slot @slotchange=${this._addTabs}></slot>`;
    }
}
